...
HEALTH=`curl --write-out '%{http_code}' --silent http://127.0.0.1:8200/v1/sys/health | tail -n1`
echo "Health is : ${HEALTH}"
until [ ${HEALTH} == "200" ]; do
 echo "Not Healthy, sleeping 5"
   sleep 5
   if [ ${HEALTH} == "503" ] || [ ${HEALTH} == "501" ] ; then
...
printf "\e[0;34m\nAdding license\n\e[0m"
curl --request PUT --header "X-Vault-Token: ${TOKEN}" -d @license.txt http://127.0.0.1:8200/v1/sys/license

sleep 5

printf "\e[0;34mCheck license is now non-temporary\n\e[0m"
curl -s --header "X-Vault-Token: ${TOKEN}" http://127.0.0.1:8200/v1/sys/license | jq '.data'
